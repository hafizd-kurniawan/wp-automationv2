...
class WordpressAPi(object):
    ...
    def __init__(self, host, jwtToken, requestsProxies=None):
        ...
        self.apiMedia = (
            self.host + "/wp-json/wp/v2/media"
        )  # 'https://www.crifan.org/wp-json/wp/v2/media'
        # https://developer.wordpress.org/rest-api/reference/posts/
        self.apiPosts = (
            self.host + "/wp-json/wp/v2/posts"
        )  # 'https://www.crifan.org/wp-json/wp/v2/posts'
        # https://developer.wordpress.org/rest-api/reference/categories/#create-a-category
        self.apiCategories = (
            self.host + "/wp-json/wp/v2/categories"
        )  # 'https://www.crifan.org/wp-json/wp/v2/categories'
        # https://developer.wordpress.org/rest-api/reference/tags/#create-a-tag
        self.apiTags = (
            self.host + "/wp-json/wp/v2/tags"
        )  # 'https://www.crifan.org/wp-json/wp/v2/tags'
    ...
    def generateUploadedImageUrl(self, uploadImageFilename):
        ...
        curDatetime = (
            datetime.now()
        )  # datetime.datetime(2021, 3, 25, 22, 42, 7, 834462)
        yearMonthStr = curDatetime.strftime(format="%Y/%m")  # '2021/03'
        uploadedImageUrl = "%s/files/pic/uploads/%s/%s" % (
            self.host,
            yearMonthStr,
            uploadImageFilename,
        )
    ...
    def createPost(
        ...
        if categoryNameList:
            # ['Mac']
            categoryIdList = self.getTaxonomyIdList(
                categoryNameList, taxonomy="category"
            )
    ...
    def createTaxonomy(self, name, taxonomy, parent=None, slug=None, description=None):
        ...
        logging.info(
            "resp=%s for POST %s with postDict=%s", resp, createTaxonomyUrl, postDict
        )
    ...
    def getTaxonomySinglePage(self, name, taxonomy, curPage, perPage=None):
        ...
        logging.debug(
            "resp=%s for GET %s with para=%s", resp, searchTaxonomyUrl, queryParamDict
        )
    ...
    def getAllTaxonomy(self, name, taxonomy):
        ...
        firstPageNum = 1
        isFirstPageSearchOk, firstPageRespInfo = self.getTaxonomySinglePage(
            name, taxonomy, firstPageNum, perPage
        )
        if isFirstPageSearchOk:
            ...
            if firstPageRespItemNum >= perPage:
                ...
                while isRestEachPageOk and isRestEachPageRespFull:
                    isRestEachPageOk, restEachPageRespItemList = (
                        self.getTaxonomySinglePage(name, taxonomy, restCurPage, perPage)
                    )
                    if isRestEachPageOk:
    ...
    def searchTaxonomy(self, name, taxonomy):
        ...
        if isGetAllOk:
            ...
            finalRespTaxonomy = crifanWordpress.findSameNameTaxonomy(
                name, respAllTaxonomyLit
            )
    ...
    def getTaxonomyIdList(self, nameList, taxonomy):
        ...
        for curIdx, eachTaxonomyName in enumerate(nameList):
            ...
            logging.debug(
                "isSearhOk=%s, existedTaxonomy=%s", isSearhOk, existedTaxonomy
            )
            ...
            if isSearhOk and existedTaxonomy:
                ...
                logging.info(
                    "Found existed %s: name=%s,id=%s,slug=%s",
                    taxonomy,
                    curTaxonomy["name"],
                    curTaxonomy["id"],
                    curTaxonomy["slug"],
                ...
                isCreateOk, createdTaxonomy = self.createTaxonomy(
                    eachTaxonomyName, taxonomy
                )
                logging.debug(
                    "isCreateOk=%s, createdTaxonomy=%s", isCreateOk, createdTaxonomy
                )
                if isCreateOk and createdTaxonomy:
                    curTaxonomy = createdTaxonomy
                    logging.info(
                        "New created %s: name=%s,id=%s,slug=%s",
                        taxonomy,
                        curTaxonomy["name"],
                        curTaxonomy["id"],
                        curTaxonomy["slug"],
            ...
            if curTaxonomy:
                ...
                logging.error(
                    "Fail search or create for %s: %s", taxonomy, eachTaxonomyName
                )

        logging.info(
            "%s nameList=%s -> taxonomyIdList=%s", taxonomy, nameList, taxonomyIdList
        )
        ...
        if resp.ok:
            ...
            if isinstance(respJson, dict):
                ...
                if "id" in respJson:
                    ...
                    logging.debug(
                        "newId=%s, newSlug=%s, newLink=%s", newId, newSlug, newLink
                    )  # newId=13224, newSlug=gpu, newLink=https://www.crifan.org/tag/gpu/
                    respInfo = {
                        "id": newId,  # 70393
                    ...
                    if "type" in respJson:
                        ...
                        if (curType == "attachment") or (curType == "post"):
                            ...
                            logging.debug(
                                "url=%s, title=%s", respInfo["url"], respInfo["title"]
                            )
